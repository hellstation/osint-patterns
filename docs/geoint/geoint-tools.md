...
The important point is not which mapping platform is considered the "best". The useful question is what information each source contributes that the others do not.
...
Temporal analysis is also useful for eliminating hypotheses. A candidate location may visually match an image, but if the relevant building, road, or landscape feature did not exist during the proposed time period, the hypothesis becomes significantly weaker.

---

## Terrain Analysis and Digital Elevation Models

Digital elevation models are among the most useful GEOINT sources because terrain generally changes much more slowly than infrastructure. SRTM and ASTER models are used to analyse elevation differences, slopes, valleys, and mountain ranges — data that can remain relevant across decades, while still requiring consideration of resolution and accuracy limitations.
...
Terrain analysis becomes especially powerful when combined with perspective. A mountain silhouette alone may produce several possible locations. Elevation profiles, camera orientation, visible valleys, roads, and structures can progressively eliminate incorrect candidates.
...
A street-level match should therefore answer a specific analytical question: does the building geometry match, does the road layout match, is the visible infrastructure consistent, or does the supposed location actually contradict the original image?
...
Other useful operations include cropping, perspective correction, channel isolation, edge enhancement, and overlaying independent images for comparison. Their purpose is not to manufacture evidence, but to make existing evidence easier to examine.
...
This distinction becomes especially important when analysing low-resolution or heavily compressed imagery. An apparent detail created by processing should never be treated as independently verified evidence.
...
Metadata should therefore be treated as another layer of evidence rather than a final answer. Embedded information can be missing, modified, stripped during upload, or affected by subsequent processing.
...
Video also provides something a still image cannot: temporal relationships. Movement of vehicles, changing shadows, weather conditions, construction activity, and the appearance of objects across consecutive frames can all provide additional evidence.

A single frame may be ambiguous. A sequence can reveal the context that makes the frame useful.
...
Astronomical tools that calculate solar position are used in shadow and lighting analysis. They allow analysts to determine whether an observed light direction is consistent with the proposed location and time of day.

A mismatch between image lighting and the astronomical record can be a strong indicator of geolocation error — but it should be treated as evidence to investigate rather than automatic proof. Terrain, orientation, cloud cover, artificial lighting, and other environmental factors can affect the observed result.

Weather and astronomical data become particularly valuable when combined with temporal imagery. A candidate location should not only look correct — the environmental conditions should also be plausible for the proposed date and time.

---

## Example Workflow: From Image to Location

The tools above become most useful when they are combined into an iterative workflow rather than used independently.

Consider an unknown photograph showing a road, several buildings, and a mountain range.

The initial analysis begins with the image itself: architecture, road markings, vegetation, terrain, signs, shadows, visible infrastructure, and perspective.

A broad search can then be performed using Google Maps, Google Earth, OpenStreetMap, and satellite imagery to identify possible regions.

Once candidate areas have been identified, terrain data and QGIS can be used to compare elevation profiles, mountain silhouettes, road orientation, and possible camera positions.

Street View or Mapillary can then be used to verify ground-level details. Historical satellite imagery can establish whether the visible structures existed during the relevant period.

Finally, metadata, weather records, and solar position can be used as additional independent checks.
...
**observation → hypothesis → independent evidence → contradiction testing → revised hypothesis → conclusion**
...
GEOINT analysis should not be treated as a search for the first location that looks correct.

A candidate location is a hypothesis.

It should accumulate supporting evidence while simultaneously being tested for contradictions.

For example:

**Hypothesis:** the image was captured at Location A.

**Supporting evidence:**

* mountain silhouette matches;
* road geometry is consistent;
* building proportions correspond;
* satellite imagery confirms the surrounding terrain.

**Contradicting evidence:**

* infrastructure did not exist during the proposed period;
* shadow direction is inconsistent;
* road markings do not match;
* weather conditions are difficult to reconcile.

The purpose of this process is not to force certainty where none exists. It is to determine which hypothesis remains most consistent with the available evidence.

A strong GEOINT conclusion therefore does not simply state *where* an image was taken. It should make clear **why** that conclusion was reached, which independent sources support it, and what uncertainty remains.

---

## Integrating Tools into the Analytical Process

No GEOINT tool is used in isolation. Satellite imagery is cross-referenced with ground-level panoramas, images are checked against metadata, and visual cues are verified against weather and temporal data.

The workflow is not linear — it is iterative, with each new source either reinforcing or complicating what came before.

The quality of analysis is determined not by the number of tools used, but by the logic of how they are applied. Each new source should either strengthen a conclusion or create grounds for doubt.

That tension — between confirmation and scepticism — is where real analysis happens.
...
Professional visual intelligence begins where the tools recede into the background — where analytical thinking, discipline, and the willingness to revise one's own conclusions become the decisive factors.

The tools are a means.

The question being asked, the evidence used to answer it, and the rigour with which that evidence is tested is everything else.
